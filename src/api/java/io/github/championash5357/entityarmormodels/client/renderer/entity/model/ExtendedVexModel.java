...
public class ExtendedVexModel extends ExtendedBipedModel<VexEntity, VexModel> {
	...
	private final Method getBodyParts;

	public ExtendedVexModel(float modelSize) {
		this(modelSize, 64, 64);
	}

	public ExtendedVexModel(float modelSize, int textureWidthIn, int textureHeightIn) {
		...
		getBodyParts = ObfuscationReflectionHelper.findMethod(VexModel.class, "func_225600_b_");
	}
	...
	public void copyAttributesOfModel(VexModel model) {
		...
		try {
			Object result = getBodyParts.invoke(model);
